...
//not sure if need to initialize Vec3f variables in constructor or they are 0 by default...
Group::Group() {
...
void Group::updateBBox() {
    ...
    _bboxCenter.x=0.0;_bboxCenter.y=0.0;_bboxCenter.z=0.0;
    _bboxSize.x=0.0;_bboxSize.y=0.0;_bboxSize.z=0.0;
    ...
    t->start();
     ...
     while((child=t->next())!=NULL) {
         //first push current globalTransformMatrix onto matrix stack
         //if child is Transform, update transform matrix
         ...
         //compute Bbox of shape and multiply by M
         if (child->isShape()) {
             ...
             if (child!=(Node*)0 && child->isIndexedFaceSet()) {
                 ...
                 //get min and max for x, y, z
                 for (int i=0; i<coords.size(); i++) {
                     int num=i%3; //use modulus to find if x y or Z
                     //x coordinate
                     if (num==0) {
                     ...
                     //z coordinate
                     if (num==2) {
                         if (coords[i]<minz) {minz=coords[i];}
             ...
             //get all 8 homogeneous coordinates
             std::vector<Vec4f> corners(8);
             std::vector<Vec4f> updated_corners;
             
             //homogenous 3D coords all have 1 as 4th entry
             for (int i=0; i<corners.size(); i++) {corners.at(i).w=1;}
             ...
             //multiply each Bbox corner by M
            ...
            //now recalculate bounding box
             ...
             for (int i=0; i<updated_corners.size(); i++) {
                 if (updated_corners.at(i).x<newMinx) {newMinx=updated_corners.at(i).x;}
                 
                 if (updated_corners.at(i).x>newMaxx) {newMaxx=updated_corners.at(i).x;}
                 
                 if (updated_corners.at(i).y<newMiny) {newMiny=updated_corners.at(i).y;}
                 
                 if (updated_corners.at(i).y>newMaxy) {newMaxy=updated_corners.at(i).y;}
                 
                 if (updated_corners.at(i).z<newMinz) {newMinz=updated_corners.at(i).z;}
                 
                 if (updated_corners.at(i).z>newMaxz) {newMaxz=updated_corners.at(i).z;}
             }
